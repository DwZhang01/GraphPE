...
def visualize_path(G, pos, path, start_node, target_node):
    ...
    # 高亮起点和终点
    nx.draw_networkx_nodes(G, pos, nodelist=start_node, node_color='green', node_size=node_size, label="Start")
    nx.draw_networkx_nodes(G, pos, nodelist=target_node, node_color='red', node_size=node_size, label="Target")
    
    edge_color_list = ['orange','red','blue','green','purple','brown']
    path_num = len(path)
    for i, path_ in enumerate(path):
        # # 绘制路径
        path_edges = list(zip(path_, path_[1:]))
        # nx.draw_networkx_edges(G, pos, edgelist=path_edges, edge_color='orange', width=3.0)
        
        nx.draw_networkx_edges(G, pos, edgelist=path_edges, edge_color=edge_color_list[i%len(edge_color_list)],width=1.5)
...
if __name__ == "__main__":
    ...
    end_node = random.sample(nodes, NUM_START_NODES+1)       
    start_node = end_node[:-1]
    target_node = end_node[-1]
    T_node = [target_node] * NUM_START_NODES
    ...
    for i in range(NUM_START_NODES):
        found_path, flag = find_path_in_embedding_space(adj, embeddings, start_node[i], target_node)
        if not flag:
            print(f"{i}:从起点 {start_node[i]} 到终点 {target_node} 的路径搜索失败。")
            F_path.append([])
        else:
            print(f"{i}:从{start_node[i]}到{target_node}的路径:{found_path}")
            F_path.append(found_path)
        
        shortest_path = nx.shortest_path(G, source=start_node[i], target=target_node)
        S_path.append(shortest_path)
        print(f"{i}:从{start_node[i]}到{target_node}的最短路径:{shortest_path}")
    ...
    # visualize_path(G, pos, found_path2, start_node2, target_node)
    # visualize_path(G, pos, shortest_path2, start_node2, target_node)
    visualize_path(G, pos, F_path, start_node, T_node)
    visualize_path(G, pos, S_path, start_node, T_node)
